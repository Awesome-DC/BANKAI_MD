...
zokou({ nomCom: "tagall", categorie: 'Group', reaction: "📣" }, async (dest, zk, commandeOptions) => {
  ...
  if (!verifGroupe) { repondre("✋🏿 ✋🏿group only mf ❌"); return; }
  if (!arg || arg === ' ') {
  mess = 'Aucun Message'
  } else {
    mess = arg.join(' ')
...
BANKAI({ nomCom: "gpp", categorie: 'Group' }, async (dest, zk, commandeOptions) => {
...
BANKAI({ nomCom: "apk", reaction: "✨", categorie: "Research" }, async (dest, zk, commandeOptions) => {
